...
namespace ProjectSL
{
    public class UserDataModel : SingletonBase<UserDataModel>
    {
        ...
        // 일단 dropitem에서 모든 정보를 ItemDataDTO.ItemData 형태로 묶은 모습
        // 이렇게 하거나, 아니면 itemID 랑 count 만 전달되고, GetItemData를 하논 과정이 UserDataModel 에서 이루어지는 방식도 문제가 없다고 생각됨
        // 그러면 어디에서 GetItemData를 호출해 주는 것이 맞을지 고민이 많이 됨
        public void AddItemData2(ItemDataDTO.ItemData itemData, int count)
        {
